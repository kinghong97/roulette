...
reader.onload = function (e) {
    ...
    sumSum(aa, bdata, dateone(date), table1)
...
function clicknone() {
    ...
    document.getElementById("firstbtn").classList.remove("hidden");
...
function makenumber(data) {
    ...
    for (i of data) {
        document.getElementById("numberbox").innerHTML +=  `<input id="number" name="number" type="number" min="0" value="${i}">${i.split(' - ')[1]} <br />`;
    }
}
...
function fristmsg1(e) {
    ...
    for (var number of numbered) {  
        if(number.value > 0){
            numberarr [number.defaultValue] = number.value
        }}
...
document.getElementById('firstbtn').addEventListener('click', fristmsg);
